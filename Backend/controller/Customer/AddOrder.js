...
export const addOrder = async (req, res) => {
  try {
    ...
    if (!order) {
      ...
      const totalAmount = order.OrderHistory.reduce((acc, order) => acc + order.total, 0);
...
export const fetchOrdersByAdminId = async (req, res) => {
  try {
    ...
    if (orders.length === 0) {
      return res.status(404).json({ message: 'No orders found for this AdminId' });
    }

    // Step 2: Iterate over orders and filter each OrderHistory by itemsStatus 'prepare'
    const filteredOrders = orders.map(order => {
      const filteredHistory = order.OrderHistory.filter(history => history.itemsStatus === 'prepare');
    ...
    if (filteredOrders.length === 0) {
      return res.status(404).json({ message: 'No orders with itemsStatus "prepare" found' });
    }
