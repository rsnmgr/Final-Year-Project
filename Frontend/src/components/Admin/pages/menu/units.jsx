...
export default function units() {
  ...
  const handleToggleModal = () => {
    ...
    if (!showModal) {
      setunitData({ name: "", status: "Active" }); // Reset unit data with default status
      setSelectedUnit(null);
    }
  };

  const handleEditClick = async (unitId) => {
    if (!AdminId) {
      toast.warn("Admin ID is not available.");
      return;
    }
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/unit/${AdminId}/${unitId}`
      );
      setunitData(response.data.unit || { name: "", status: "Active" });
  ...
  const handleDeleteConfirm = async () => {
    if (!AdminId || !selectedUnit) {
      toast.warn("Admin ID or selected unit is not available.");
      return;
    }

    try {
      await axios.delete(
        `${API_URL}/api/unit/${AdminId}/${selectedUnit}`
      );
      setUnits((prevUnits) =>
        prevUnits.filter((unit) => unit._id !== selectedUnit)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedUnit) {
        ...
        await axios.post(`${API_URL}/api/unit`, {
          AdminId,
          ...unitData,
        });
